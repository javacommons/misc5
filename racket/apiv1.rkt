...
(define-ffi-definer define-apiv1 (ffi-lib (format "apiv1-~abit" (system-type 'word))))
(define-apiv1 make_client_pipe (_fun _string _string _int -> _pointer))
(define-apiv1 make_server_pipe (_fun _string -> _pointer))
(define-apiv1 read_from_pipe(_fun _pointer -> _string))
(define-apiv1 write_to_pipe(_fun _pointer _string -> _void))
...
(define (::open-pipe-server $name $client $show-client)
  (make_client_pipe $name $client $show-client))

(define (::open-pipe-client $name)
  (make_server_pipe $name))
