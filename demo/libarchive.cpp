...
{
    auto copy = args;
    copy["api"] = "api_archive_next_entry";
    struct archive *a = (struct archive *)get_handle_addr(args, "archive");
    ...
    if (r == ARCHIVE_EOF)
    {
        formatA(std::cout, "api_archive_next_entry(2)\n");
        formatA(std::cout, "(r == ARCHIVE_EOF)\n");
        return false;
    }
    if (r < ARCHIVE_OK)
    {
        formatA(std::cout, "api_archive_next_entry(3)\n");
        formatA(std::cout, "(r < ARCHIVE_WARN): %s\n", archive_error_string(a));
        return false;
    }
    if (r < ARCHIVE_WARN)
    {
        formatA(std::cout, "api_archive_next_entry(4)\n");
        formatA(std::cout, "(r < ARCHIVE_WARN)\n");
        return false;
    }
#endif
